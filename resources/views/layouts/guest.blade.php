...
<body class="font-sans antialiased text-[var(--flux-ink)]">
    <div class="flex min-h-screen flex-col items-center bg-[var(--flux-bg)] pt-6 sm:justify-center sm:pt-0">
        <div>
            <a href="/">
                <x-application-logo class="h-20 w-20 fill-current text-[var(--flux-dark)]" />
            </a>
        </div>

        <div class="mt-6 w-full overflow-hidden rounded-2xl border border-[var(--flux-border)] bg-white px-6 py-4 shadow-sm sm:max-w-md">
            {{ $slot }}
        </div>
    </div>
</body>
