<x-app-layout>
    ...
    @php
        $selectedType = old('type', $defaultType ?? null);
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @if ($organizations->isEmpty())
                        <div class="mb-4 rounded-md bg-yellow-100 text-yellow-800 px-4 py-2">
                            Nenhuma organizacao encontrada. Cadastre uma organizacao antes de criar categoria.
                        </div>
                    @endif
                    ...
                    <form method="POST" action="{{ route('categories.store') }}" class="space-y-4">
                        ...
                        <div id="cost_type_wrapper" @class(['hidden' => ! $showCostType])>
                            ...
                            <select id="cost_type" name="cost_type"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="">Selecione...</option>
                                <option value="fixed" @selected(old('cost_type', $defaultCostType ?? null) === 'fixed')>Fixo</option>
                                <option value="variable" @selected(old('cost_type', $defaultCostType ?? null) === 'variable')>Variavel</option>
                                <option value="income" hidden @selected(old('cost_type', $defaultCostType ?? null) === 'income')>Receita</option>
                            </select>
